...
<Card title="Rhythm" class="border-l-4 border-l-orange-500">
	<!-- Mode Toggle -->
	<div class="mb-5">
		<ToggleGroup
			type="single"
			value={localModeString}
			onValueChange={handleModeChange}
			class="w-full justify-stretch bg-neutral-800"
		>
			<ToggleGroupItem value="0" class="flex-1 data-[state=on]:bg-orange-600"
				>Euclidean</ToggleGroupItem
			>
			<ToggleGroupItem value="1" class="flex-1 data-[state=on]:bg-purple-600"
				>PerfectBalance</ToggleGroupItem
			>
			<ToggleGroupItem value="2" class="flex-1 data-[state=on]:bg-teal-600"
				>ClassicGroove</ToggleGroupItem
			>
		</ToggleGroup>
	</div>
	...
	{#if localMode === 0}
		<!-- EUCLIDEAN MODE -->
		<p class="mb-4 text-xs text-neutral-500">Bjorklund algorithm - Classic polyrhythms</p>

		<!-- Primary Sequencer -->
		<div class="mb-4 rounded-lg bg-neutral-800/50 p-3">
			<div class="mb-2 text-xs font-semibold text-orange-300">Primary (Kick)</div>
		...
		<!-- Secondary Sequencer -->
		<div class="rounded-lg border border-green-500/30 bg-neutral-800/50 p-3">
			<div class="mb-2 text-xs font-semibold text-green-300">Secondary (Snare) - Polyrhythm</div>
			<div class="grid grid-cols-3 gap-3">
				<Slider
	...
	{:else if localMode === 1}
		<!-- PERFECTBALANCE MODE -->
		<p class="mb-4 text-xs text-neutral-500">XronoMorph style - Regular polygons</p>

		<!-- Poly Steps Selection -->
		<div class="mb-4">
			<span class="mb-2 block text-xs text-neutral-400">Resolution (steps per measure)</span>
	...
	{:else}
		<!-- CLASSICGROOVE MODE -->
		<p class="mb-4 text-xs text-neutral-500">Realistic drum patterns - Ghost notes & grooves</p>
