...
export class WasmBridge extends BaseBridge {
	...
	protected sendCommand(method: string, ...args: unknown[]): void {
		if (
			this.handle &&
			typeof (this.handle as unknown as Record<string, (...args: unknown[]) => void>)[method] ===
				'function'
		) {
			(this.handle as unknown as Record<string, (...args: unknown[]) => void>)[method](...args);
		}
	}
	...
	private collectState(): boolean {
		...
		const updateArray = <K extends keyof EngineState>(
			key: K,
			newValue: BigInt64Array | Uint8Array | Float32Array | boolean[] | number[]
		) => {
			...
			if (
				newValue instanceof BigInt64Array ||
				...
				newArray = newValue as (number | boolean)[];
			}
