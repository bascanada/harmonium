...
/**
 * Represents a single note event for real-time visualization.
 * Generated by the audio engine and consumed by LiveScore and Tonnetz components.
 */
export interface NoteEvent {
	note: number; // MIDI note number (0-127)
	instrument: number; // Instrument channel: 0=Bass, 1=Lead, 2=Snare, 3=Hat
	step: number; // Step index when the event occurred
	duration: number; // Duration in samples
	timestamp: number; // High-precision timestamp (performance.now())
}

/**
 * Event stream container for buffering and managing note events.
 */
export interface EventStream {
	events: NoteEvent[];
	lastUpdate: number; // Timestamp of last event batch
}
