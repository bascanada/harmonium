...
pub const MAX_ACCENT_LEN: usize = 16;
...
impl Polygon {
    /// Standard constructor (uniform velocity, no per-vertex accents).
    #[must_use]
    pub const fn new(vertices: usize, rotation_offset: usize, velocity: f32) -> Self {
        Self { vertices, rotation_offset, velocity, accent_pattern: AccentPattern::empty() }
    }

    /// Constructor with a per-vertex accent pattern.
    #[must_use]
    pub fn with_accents(
        vertices: usize,
        ...
        Self { vertices, rotation_offset, velocity, accent_pattern }
    }
    ...
    /// Same logic as `hits()` but also computes which vertex number this step
    /// corresponds to (0-based). If not a hit, `vertex_index` is 0.
    #[must_use]
    pub const fn hit_info(&self, step: usize, total_steps: usize) -> (bool, usize) {
        if self.vertices == 0 || total_steps == 0 || self.vertices > total_steps {
            return (false, 0);
        }
        ...
        let is_hit = if total_steps.is_multiple_of(self.vertices) {
            let interval = total_steps / self.vertices;
            adjusted_step.is_multiple_of(interval)
        } else {
            let prev_adjusted_step = (adjusted_step + total_steps - 1) % total_steps;
        ...
        // Compute vertex index: which vertex (0-based) does this hit correspond to?
        let vertex_index = if total_steps.is_multiple_of(self.vertices) {
            let interval = total_steps / self.vertices;
            adjusted_step / interval
        } else {
            // For Euclidean rhythms, the vertex index is the pulse number at this step.
            (adjusted_step * self.vertices) / total_steps
        };
...
pub fn generate_balanced_layers(
    ...
    for (i, trigger) in triggers.iter_mut().enumerate().take(steps) {
        let (hit_kick, kick_vi) = kick_gon.hit_info(i, steps);
        let (hit_snare, snare_vi) = snare_gon.hit_info(i, steps);
        let (hit_hat, hat_vi) = hat_gon.hit_info(i, steps);
        let (hit_bass, _) = bass_gon.hit_info(i, steps);
        let (hit_lead, _) = lead_gon.hit_info(i, steps);

        // --- 3. CONFLICT RESOLUTION & VELOCITY ---

        if hit_kick {
            trigger.kick = true;
            trigger.velocity = kick_gon.velocity_at(kick_vi);
        }

        if hit_snare {
            trigger.snare = true;
            // If snare hits same time as kick, accentuate it
            trigger.velocity = if hit_kick { 1.0 } else { snare_gon.velocity_at(snare_vi) };
        }
        ...
        if hit_hat {
            ...
            if !mask {
                ...
                if !hit_kick && !hit_snare {
                    trigger.velocity = hat_gon.velocity_at(hat_vi);
                }
            }
        }
