...
use odin2_core::{
    SynthEngine,
    engine::{OdinEngine, PresetConfig},
    preset::OdinPreset,
};
...
impl InstrumentType {
    ...
    const fn from_channel(channel: u8) -> Self {
        match channel {
            0 => Self::Bass,
            1 => Self::Lead,
            2 => Self::Snare,
...
impl QuadrantConfigs {
    ...
    pub fn load_defaults(category: &str, names: [&str; 4]) -> Self {
        ...
        let load_embedded = |name: &str| -> Option<PresetConfig> {
            let bytes: Option<&[u8]> = match (category, name) {
                // Keys
                ("Keys", "Synth Piano") => {
                    Some(include_bytes!("../../../assets/odin2/Keys/Synth Piano.odin"))
                }
                ("Keys", "Toy Piano") => {
                    Some(include_bytes!("../../../assets/odin2/Keys/Toy Piano.odin"))
                }
                ("Keys", "Pianet") => {
                    Some(include_bytes!("../../../assets/odin2/Keys/Pianet.odin"))
                }
                ("Keys", "Piano Ballad 3") => {
                    Some(include_bytes!("../../../assets/odin2/Keys/Piano Ballad 3.odin"))
                }

                // Drums
                ("Drums", "Kick-1 [Photonic]") => {
                    Some(include_bytes!("../../../assets/odin2/Drums/Kick-1 [Photonic].odin"))
                }
                ("Drums", "Snare-1 [Photonic]") => {
                    Some(include_bytes!("../../../assets/odin2/Drums/Snare-1 [Photonic].odin"))
                }
                ("Drums", "Drum Machine") => {
                    Some(include_bytes!("../../../assets/odin2/Drums/Drum Machine.odin"))
                }
                ("Drums", "HiHat-closed [Photonic]") => {
                    Some(include_bytes!("../../../assets/odin2/Drums/HiHat-closed [Photonic].odin"))
                }

                // Bass
                ("Bass", "Bass Crusher [RM]") => {
                    Some(include_bytes!("../../../assets/odin2/Bass/Bass Crusher [RM].odin"))
                }
                ("Bass", "Bass Simple PM [RS]") => {
                    Some(include_bytes!("../../../assets/odin2/Bass/Bass Simple PM [RS].odin"))
                }
                ("Bass", "DeepBass [Photonic]") => {
                    Some(include_bytes!("../../../assets/odin2/Bass/DeepBass [Photonic].odin"))
                }
                ("Bass", "Analog Bass [tx]") => {
                    Some(include_bytes!("../../../assets/odin2/Bass/Analog Bass [tx].odin"))
                }
            ...
            if let Some(data) = bytes {
                if let Ok(preset) = OdinPreset::from_bytes(data) {
                    harmonium_core::log::info(&format!(
                        "Loaded embedded preset: {category}/{name}"
                    ));
                    return Some(PresetConfig::from_preset(&preset));
                }
                harmonium_core::log::error(&format!(
                    "Failed to parse embedded preset: {category}/{name}"
                ));
            }
            None
        };
        ...
        let load = |name: &str| -> PresetConfig {
            ...
            for base in &base_paths {
                let path = format!("{base}/{category}/{name}.odin");
            ...
            harmonium_core::log::error(&format!(
                "Failed to load preset: {category}/{name}.odin - Using Fallback"
            ));
    ...
    pub fn morph(&self, x: f32, y: f32) -> PresetConfig {
        ...
        PresetConfig {
            ...
            filter_frequency: mix(
                p1.filter_frequency,
                p2.filter_frequency,
                p3.filter_frequency,
                p4.filter_frequency,
            ),
            filter_resonance: mix(
                p1.filter_resonance,
                p2.filter_resonance,
                p3.filter_resonance,
                p4.filter_resonance,
            ),
            filter_env_amount: mix(
                p1.filter_env_amount,
                p2.filter_env_amount,
                p3.filter_env_amount,
                p4.filter_env_amount,
            ),
...
impl Odin2Backend {
    ...
    pub fn new(sample_rate: f64) -> Self {
        ...
        let bass_names =
            ["Bass Crusher [RM]", "Bass Simple PM [RS]", "DeepBass [Photonic]", "Analog Bass [tx]"];
    ...
    pub const fn set_gains(&mut self, lead: f32, bass: f32, snare: f32, hat: f32) {
    ...
    pub fn apply_emotional_morphing(
        &mut self,
        valence: f32,
        arousal: f32,
        _tension: f32,
