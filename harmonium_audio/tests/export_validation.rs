use harmonium_audio::backend::{AudioRenderer, recorder::RecorderBackend, synth_backend::SynthBackend};
use harmonium_core::{
...
fn test_export_roundtrip_validation() {
    ...
    // 2. Verify Truth
    let truth_data = recordings.iter().find(|(f, _)| *f == RecordFormat::Truth).map(|(_, d)| d).expect("Truth missing");
    let truth: RecordingTruth = serde_json::from_slice(truth_data).expect("Failed to parse Truth");
    
    // 3. Verify MusicXML
    let xml_data = recordings.iter().find(|(f, _)| *f == RecordFormat::MusicXml).map(|(_, d)| d).expect("MusicXML missing");
    let xml_str = std::str::from_utf8(xml_data).unwrap();
    let xml_str_clean = xml_str.lines().filter(|l| !l.contains("!DOCTYPE")).collect::<Vec<_>>().join("\n");
    let doc = Document::parse(&xml_str_clean).expect("Invalid MusicXML");
    
    let note_elements: Vec<_> = doc.descendants().filter(|n| n.has_tag_name("note") && !n.children().any(|c| c.has_tag_name("rest"))).collect();
    let truth_notes = truth.events.iter().filter(|(_, e)| matches!(e, AudioEvent::NoteOn { velocity, .. } if *velocity > 0)).count();
    
    assert_eq!(note_elements.len(), truth_notes, "Note count mismatch over 30s export");

    // 4. Verify Audio (WAV)
    let wav_data = recordings.iter().find(|(f, _)| *f == RecordFormat::Wav).map(|(_, d)| d).unwrap();
    ...
    let rms_silence = (silence_seg.iter().map(|s| s * s).sum::<f32>() / silence_seg.len() as f32).sqrt();
    let rms_signal = (signal_seg.iter().map(|s| s * s).sum::<f32>() / signal_seg.len() as f32).sqrt();
...
fn run_30s_complex_validation_test(sample_rate: u32) -> Vec<(RecordFormat, Vec<u8>)> {
    ...
    let sf2_data = std::fs::read(sf2_path).ok();
    
    let inner = Box::new(SynthBackend::new(sample_rate as f64, sf2_data.as_deref(), &vec![-1; 16]));
    let mut recorder = RecorderBackend::new(inner, finished_recordings.clone(), sample_rate);

    // Warm up
    let mut buffer = vec![0.0f32; 4096];
    for _ in 0..10 { recorder.process_buffer(&mut buffer, 2); }
