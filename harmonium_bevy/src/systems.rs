...
use crate::{
    assets::OdinAsset,
    components::{AiDriver, HarmoniumSource, HarmoniumTag},
    Harmonium,
};
...
pub fn sync_harmonium_params(
    ...
    if let Some(source) = query.iter().next() {
        ...
        // density -> pulses
        k_params.rhythm_pulses =
            (source.config.density * source.config.steps as f32).round() as usize;
...
pub fn scan_environment_system(
    ...
    let manual_params: harmonium_core::EngineParams =
    ...
    for (mut driver, player_transform) in &mut driver_query {
        ...
        let final_arousal = manual_params.arousal.mul_add(1.0 - mix, ai_target.arousal * mix);
        let final_valence = manual_params.valence.mul_add(1.0 - mix, ai_target.valence * mix);
        let final_tension = manual_params.tension.mul_add(1.0 - mix, ai_target.tension * mix);
        ...
        harmonium.params.arousal =
            (final_arousal - harmonium.params.arousal).mul_add(f, harmonium.params.arousal);
        harmonium.params.valence =
            (final_valence - harmonium.params.valence).mul_add(f, harmonium.params.valence);
        harmonium.params.tension =
            (final_tension - harmonium.params.tension).mul_add(f, harmonium.params.tension);
    }
}
